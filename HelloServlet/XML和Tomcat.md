...
* DOM：（document object model）会把整个文档全部读到内存当中，形成树状结构。能够把这个树状结构抽取出来，
文档为 docuement  元素 element 属性 attribute 文本 text 
这四个对象统称为 Node 
	如果Xml特别大，会造成内存溢出。可以对文档进行增删操作。
	
* SAX（Simple Api for Xml） 基于事件驱动 读一行，解析一行。  
	不会造成内存溢出，不能进行增删，只能查询 （有点像指针的样子。）
